...
def test_apply_local_clifford_to_planner_meas_basis_xy_1(zx_graph: ZXGraphState, rng: np.random.Generator) -> None:
    ...
    ref_meas_basis = PlannerMeasBasis(Plane.XY, angle + np.pi / 2)
    ref_zx_graph.assign_meas_basis(node, ref_meas_basis)
    ref_meas_vector = ref_zx_graph.meas_bases[node].vector()
    assert np.isclose(abs(np.vdot(meas_vector, ref_meas_vector)), 1)
    assert zx_graph.meas_bases[node].plane == ref_meas_basis.plane
    assert is_close_angle(zx_graph.meas_bases[node].angle, ref_meas_basis.angle)
...
def test_apply_local_clifford_to_planner_meas_basis_xy_2(zx_graph: ZXGraphState, rng: np.random.Generator) -> None:
    ...
    ref_meas_basis = PlannerMeasBasis(Plane.XZ, angle + np.pi / 2)
    ref_zx_graph.assign_meas_basis(node, ref_meas_basis)
    ref_meas_vector = ref_zx_graph.meas_bases[node].vector()
    assert np.isclose(abs(np.vdot(meas_vector, ref_meas_vector)), 1)
    assert zx_graph.meas_bases[node].plane == ref_meas_basis.plane
...
def test_apply_local_clifford_to_planner_meas_basis_xy_3(zx_graph: ZXGraphState, rng: np.random.Generator) -> None:
    ...
    ref_meas_basis = PlannerMeasBasis(Plane.YZ, -angle)
    ref_zx_graph.assign_meas_basis(node, ref_meas_basis)
    ref_meas_vector = ref_zx_graph.meas_bases[node].vector()
    assert np.isclose(abs(np.vdot(meas_vector, ref_meas_vector)), 1)
    assert zx_graph.meas_bases[node].plane == ref_meas_basis.plane
...
def test_apply_local_clifford_to_planner_meas_basis_yz_1(zx_graph: ZXGraphState, rng: np.random.Generator) -> None:
    ...
    ref_meas_basis = PlannerMeasBasis(Plane.XZ, -angle)
    ref_zx_graph.assign_meas_basis(node, ref_meas_basis)
    ref_meas_vector = ref_zx_graph.meas_bases[node].vector()
    assert np.isclose(abs(np.vdot(meas_vector, ref_meas_vector)), 1)
    assert zx_graph.meas_bases[node].plane == ref_meas_basis.plane
...
def test_apply_local_clifford_to_planner_meas_basis_yz_2(zx_graph: ZXGraphState, rng: np.random.Generator) -> None:
    ...
    ref_meas_basis = PlannerMeasBasis(Plane.YZ, angle + np.pi / 2)
    ref_zx_graph.assign_meas_basis(node, ref_meas_basis)
    ref_meas_vector = ref_zx_graph.meas_bases[node].vector()
    assert np.isclose(abs(np.vdot(meas_vector, ref_meas_vector)), 1)
    assert zx_graph.meas_bases[node].plane == ref_meas_basis.plane
...
def test_apply_local_clifford_to_planner_meas_basis_yz_3(zx_graph: ZXGraphState, rng: np.random.Generator) -> None:
    ...
    ref_meas_basis = PlannerMeasBasis(Plane.XY, -angle)
    ref_zx_graph.assign_meas_basis(node, ref_meas_basis)
    ref_meas_vector = ref_zx_graph.meas_bases[node].vector()
    assert np.isclose(abs(np.vdot(meas_vector, ref_meas_vector)), 1)
    assert zx_graph.meas_bases[node].plane == ref_meas_basis.plane
...
def test_expand_input_local_cliffords_xy_plane(zx_graph: ZXGraphState, gamma: float) -> None:
    ...
    assert is_close_angle(zx_graph.meas_bases[new_input_node + 1].angle, 0.0)
    assert is_close_angle(zx_graph.meas_bases[old_input_node].angle, old_input_angle - gamma)
...
def test_expand_input_local_cliffords_yz_plane(zx_graph: ZXGraphState, gamma: float) -> None:
...
def test_expand_input_local_cliffords_xz_plane(zx_graph: ZXGraphState, gamma: float) -> None:
...
def test_expand_output_local_cliffords(zx_graph: ZXGraphState) -> None:
    ...
    assert zx_graph.meas_bases[old_output_node + 1].plane == Plane.XY
    assert is_close_angle(zx_graph.meas_bases[old_output_node + 1].angle, 0.0)
    meas_basis = update_lc_basis(lc, PlannerMeasBasis(Plane.XY, 0.0))
    assert zx_graph.meas_bases[old_output_node].plane == meas_basis.plane
    assert is_close_angle(zx_graph.meas_bases[old_output_node].angle, meas_basis.angle)
...
def test_remove_clifford_fails_with_input_node(zx_graph: ZXGraphState) -> None:
    node = zx_graph.add_physical_node()
    zx_graph.register_input(node, q_index=0)
    with pytest.raises(ValueError, match="Clifford node removal not allowed for input or output nodes"):
