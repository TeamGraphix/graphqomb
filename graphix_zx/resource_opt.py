...
def get_subgraph_sequences(
    graph: GraphState, meas_order: list[int]
) -> list[GraphState]:
...
def get_minimized_sp_meas_order(graph: GraphState, gflow: GFlow) -> list[int]:
    ...
    unmeasured_nodes = (
        graph.get_physical_nodes() - activated_nodes - set(graph.output_nodes)
    )
    ...
    while unmeasured_nodes:
        ...
        activation_costs = {
            node: count_activation_cost(graph, node, activated_nodes)
            for node in meas_candidates
        }
        ...
        activated_nodes |= get_activation_nodes(graph, target_node, activated_nodes) | {
            target_node
        }
...
def get_activation_nodes(
    graph: GraphState, target_node: int, activated_nodes: set[int]
) -> set[int]:
